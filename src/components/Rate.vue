  ...
  <div id="rate_contents">
    ...
    <div class="container" v-if="((content_type_tab_string != '[filter]') ? (store.rate.visible_cards.length
        ...
        <ContentCard
          ...
          :potential_rating="''"
          :where_to_watch="card_data.where_to_watch"
    ...
    <transition appear
      ...
      <button v-if="is_mobile && show_last_card_button && this.$store.state.last_card"
              :style="`--top:`+(((window_height-card_height)/2)+10)+`px;
    ...
    <transition appear
                enter-active-class="animated fadeIn"
                leave-active-class="animated fadeOut">
        <div class="rate-item-platforms"
             :style="`--bottom:`+(((window_height-card_height)/2)+5)+`px;
                      --left:`+(((window_width-(card_width-20))/2)-11.5)+`px;
              ...
              v-if="!removingCard
                    ...
                    (store.suggestions.rate_counter_all>25)
                    &&
                    (Object.keys(store.rate.visible_cards[0].where_to_watch || {}).length != 0)">
            <div class="rate-item-platforms-container"
                v-for="link, index in store.rate.visible_cards[0].where_to_watch">
                <div @click="goToPlatform(link, store.rate.visible_cards[0].content_id, 'swipe_poster')"
...
export default {
  ...
  computed: {
    ...
    content_type_tab_string() {
      return JSON.stringify(this.$store.state.rate.content_type_tab).replace(
        /['"]+/g,
        ""
      );
    }
  ...
  watch: {
    ...
    filtered_cards: {
      handler(cards) {
        if (
          cards.length <= 10 &&
          this.$store.state.rate_filters.content_ids.length != 0
        ) {
          var self = this;
          axios
            .post(self.$store.state.api_host + "get_filtered_content", {
              session_id: self.$store.state.session_id,
              content_ids: self.$store.state.rate_filters.content_ids.slice(
                0,
                25
              )
            })
            .then(function(response) {
              if ([200].includes(response.status)) {
                self.$store.state.rate_filters.visible_cards.push(
    ...
    showLastCard() {
      ...
      if (this.$store.state.last_card) {
        ...
        if (this.content_type_tab_string != "[filter]") {
          ...
          this.$store.state.rate_filters.visible_cards.unshift(
            this.$store.state.last_card
          );
        }
      }
